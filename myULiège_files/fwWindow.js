...
function fwModalConfirm_create(text, textOk, iconOk, jsOk, textCancel, iconCancel, jsCancel, text3, icon3, js3, iconSize, titre, small, alert, enterEqualOk, escEqualCancel, withCheckForm,
		as_idConfirm)
{
	var idConfirm;
	if (as_idConfirm)
...
function fwWindow_stoptDragging()
{
	if (fwWindowCurrentDrag != undefined)
	{
		...
		$("#" + fwWindow_getIdOfContent(fwWindowCurrentDrag.attr("fwWindowId"))).show();
...
function fwWindow_drag(e)
{
	if (fwWindowCurrentDrag != undefined)
	{
		var vTop = e.clientY - fwWindowCurrentDragY;
		if (vTop < 0)
			vTop = 0;
		var vLeft = e.clientX - fwWindowCurrentDragX;
		if (vLeft < 0)
			vLeft = 0;
		fwWindowCurrentDrag.css("top", vTop);
		fwWindowCurrentDrag.css("left", vLeft);
	}
}
...
function fwWindow_startDragging(e)
{
	...
	if (fwWindowCurrentDrag.attr("fwDrag") == "true")
	{
		...
		$(".fwWindowContentAll").css("z-index", "100001");
		fwWindowCurrentDrag.css("z-index", "100002");
		$("#" + fwWindow_getIdOfContent(fwWindowCurrentDrag.attr("fwWindowId"))).hide();
	}
	else
	{
		fwWindowCurrentDrag = undefined;
...
function fwWindow_createIfNeeded(id, width, height, url, titre, closeable, maximizable, resizeable, minWidth, minHeight, maxWidth, maxHeight)
{
	...
	if (!ret.length)
	{
		...
		if (url)
		{
			var divWait = $("<div />");
			var divLoading = $("<div />");
			divLoading.addClass("fw_loading fw_loading64");
			divWait.append(divLoading);
			divContent.append(divWait);
			var iframe = $("<iframe />");
			iframe.attr("width", "100%");
			iframe.attr("height", "100%");
			iframe.attr("src", url);
			divContent.append(iframe);
			divWait.css("padding-top", "" + ((height / 2) - 32 - 12) + "px");
			divWait.css("padding-left", "" + ((width / 2) - 32) + "px");
			iframe.bind("load", function()
			{
				divWait.hide();
			});
		...
		$("#contenuPrincipalBody").append(ret);
	}
	return ret;
}
